    ...
    for i in 1:1:num
        ...
        for j in 1:1:num    
            if i != j
                ...
                # Use mixing rules
                sigma = (parts[i].sigma + parts[j].sigma) / 2.0
                ...
                potential += lj_potential(sigma, epsilon, dist) / 2.0
            end
        end
        ...
        newparts[i].velocity.x = (newparts[i].position.x - oldparts[i].position.x) / tdt
        newparts[i].velocity.y = (newparts[i].position.y - oldparts[i].position.y) / tdt
        newparts[i].velocity.z = (newparts[i].position.z - oldparts[i].position.z) / tdt

        kinetic += (newparts[i].velocity.x^2 + newparts[i].velocity.y^2 +
            newparts[i].velocity.z^2) * newparts[i].mass / 2.0
    end
...
function main()
    ...
    for _ in start:timestep:stop
        ...
        write(file, "$num \n")
        write(file, "\n")
        for (_, val) in enumerate(particles)
